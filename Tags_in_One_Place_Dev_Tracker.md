...
- **目标路径解析/校验**: 规范化分隔符；拒绝绝对路径/`..`/尾随 `/`；缺省扩展名自动补 `.md`（`src/path-utils.ts`）
- **写入优化**: 写入前比较内容，忽略时间戳行差异；无变化则跳过写盘（`src/file-writer.ts`）
- **递归创建父目录**: 支持多级目录目标路径（如 `a/b/c/Tags.md`），逐级创建中间文件夹（`src/file-writer.ts`）
...
├── path-utils.ts     # 目标路径解析/校验：补 .md、拒绝绝对路径/.. 等
...
### 3.7 P0.2 多级父目录创建不确定（可能导致写入失败）- 已修复
此前风险：只对 `folderPath` 调用一次 `createFolder(folderPath)`，路径如 `a/b/c/Tags.md` 在 `a`、`a/b` 不存在时可能失败。  
当前实现：写入前对父目录逐级检查/创建，确保中间目录存在（`src/file-writer.ts`）。

### 3.8 P0.3 目标路径缺少严格校验/规范化策略（健壮性）- 已修复（核心场景）
此前风险：仅 `trim + normalizePath`，对空路径、尾随 `/`、`..`、反斜杠、绝对路径等输入缺少明确策略。  
当前实现：集中解析/校验目标路径；支持缺省扩展名自动补 `.md`；非法输入会在运行命令时失败并提示原因（`src/path-utils.ts` + `src/main.ts`）。

### 3.9 P1.3 metadata cache 缺失提示可操作性不足 - 已改善
此前风险：只提示缺失数量，用户不清楚如何处理。  
当前实现：当缺失比例过高或缺失数量过大时，Notice 会建议稍后重试一次（`src/main.ts`）。
...
当前已有“缺失数量提示”，并在缺失过多时建议用户稍后重试一次；但仍没有等待 cache 就绪/自动重试的机制。
...
### P0（避免失败/正确性）
- （已完成）递归创建父目录（支持多级目录目标路径）
- （已完成）补齐目标路径校验：空值、`..`、尾随 `/`、反斜杠、绝对路径等；缺省扩展名自动补 `.md`

### P1（可靠性/性能）
- 针对 metadata cache 缺失：目前仅在缺失过多时提示用户稍后重试；未来可选“等待 cache 构建完成后再运行”或“自动重试 N 次”
